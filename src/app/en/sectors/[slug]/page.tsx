...
export async function generateStaticParams() {
    const sectors = getAllMDXMetadata("sectors", "en");
    return sectors.map((sector) => ({ slug: sector.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    const { slug } = await params;
    const data = await getMDXBySlug(slug, "sectors", "en");
    if (!data) return {};
    return {
        title: data.frontmatter.title,
        description: data.frontmatter.description,
        alternates: {
            canonical: `https://eosproje.com/en/sectors/${slug}`,
            languages: {
                'en': `/en/sectors/${slug}`,
                'tr': `/sektorler/${slug}`,
            },
        ...
        <div className="min-h-screen bg-white">
            <SubPageHero
                ...
                breadcrumb={[
                    ...
                    { label: frontmatter.title, href: `/en/sectors/${slug}` }
                ]}
            />
            ...
            <JsonLd data={{
                ...
                "serviceType": frontmatter.category || "Laser Scanning and BIM",
                "areaServed": ["TR", "HU", "EU"],
                "image": frontmatter.image ? `https://eosproje.com${frontmatter.image}` : undefined,
                "mainEntityOfPage": {
                    "@type": "WebPage",
                    "@id": `https://eosproje.com/en/sectors/${slug}`
                }
            }} />
            ...
            <div className="max-w-4xl mx-auto px-4 py-16">
                ...
                {frontmatter.faq && frontmatter.faq.length > 0 && (
                    <div className="mt-20 bg-slate-900 rounded-[2.5rem] p-8 md:p-12 text-white overflow-hidden relative">
                        <div className="relative z-10">
                            <h3 className="text-2xl md:text-3xl font-black mb-8 text-center">Frequently Asked Questions</h3>
                ...
                <div className="mt-16 text-center">
                    <Link href="/en/contact" className="inline-flex items-center gap-3 px-10 py-4 bg-[var(--color-primary-red)] text-white font-black uppercase tracking-widest text-sm rounded-2xl hover:bg-red-800 transition-all shadow-xl shadow-red-900/30 hover:-translate-y-1">
                        Get Detailed Information <ArrowRight className="w-5 h-5" />
                    </Link>
                </div>
