...
export async function generateStaticParams() {
    const posts = (await import("@/lib/mdx")).getAllMDXMetadata("blog", "en");
...
export default async function BlogPostPageEn({ params }: PageProps) {
    const { slug } = await params;
    const post = await getMDXBySlug(slug, "blog", "en");

    if (!post) notFound();

    const { frontmatter, content, readingTime } = post;
    const relatedPosts = getRelatedPosts(slug, "blog", "en", 3);
        ...
        <div className="min-h-screen bg-white">
            <SubPageHero
                ...
                breadcrumb={[
                    ...
                    { label: frontmatter.title, href: `/en/blog/${slug}` }
                ]}
            />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                        ...
                        <article>
                            <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 mb-6 font-medium">
                                ...
                                <span className="flex items-center gap-1.5">
                                    ...
                                    {new Date(frontmatter.date).toLocaleDateString("en-US", { day: "numeric", month: "long", year: "numeric" })}
                                </span>
                                <span className="flex items-center gap-1.5">
                    ...
                    <aside className="lg:col-span-4 space-y-10">
                        <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 text-center sticky top-24 transition-all">
                            <div className="w-24 h-24 bg-slate-200 rounded-full mx-auto mb-4 overflow-hidden border-4 border-white shadow-md relative text-center">
                            ...
                            <p className="text-xs font-black text-[var(--color-primary-red)] uppercase tracking-widest mb-4">
                                {AUTHORS[frontmatter.author as string]?.role || "BIM Specialist"}
                            </p>
                            <p className="text-sm text-slate-500 font-medium leading-relaxed mb-6">
                                {AUTHORS[frontmatter.author as string]?.description || "Specialist team member focused on BIM and laser scanning solutions at Eos Proje."}
                            </p>
                        ...
                        <div className="bg-slate-900 p-8 rounded-[2rem] text-white text-center relative overflow-hidden group">
                            <div className="relative z-10">
                                <h3 className="text-xl font-black mb-4">Start Your Project</h3>
                                <p className="text-slate-400 text-sm mb-8 font-medium">Consult with our experts for tailored solutions to your needs.</p>
                                <Link href="/en/contact" className="block w-full py-3 bg-[var(--color-primary-red)] rounded-xl font-bold text-sm hover:bg-red-700 transition-colors shadow-lg shadow-red-900/40 transform group-hover:scale-[1.02] transition-transform">
                                    Get a Quote
                                </Link>
                            </div>
