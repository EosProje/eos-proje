...
export default function SectorListClient({ relatedProjectsMetadata }: { relatedProjectsMetadata: any[] }) {
    ...
    const currentSectorData = sectors.find(s => s.id === activeSector)!;
    const relatedProjects = relatedProjectsMetadata
        .filter(p => p.sector?.toLowerCase().includes(currentSectorData.title.toLowerCase().split(' ')[0].toLowerCase()))
        .slice(0, 3);
    ...
    return (
        <div className="min-h-screen bg-white">
            <SubPageHero
                ...
                description="Endüstriyel tesislerden tarihi yapılara kadar her alanda profesyonel BIM ve modelleme hizmetleri sunuyoruz."
                category="Sektörler"
                breadcrumb={[{ label: "Sektörler", href: "/sektorler" }]}
            />
            ...
            <div className="max-w-7xl mx-auto px-4 py-24 relative">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
                    ...
                    <div className="lg:col-span-4 space-y-4">
                        <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-8 flex items-center gap-2">
                            <MapPin className="w-3 h-3" /> Uzmanlık Alanı Seçin
                        </h3>
                                ...
                                <div className="relative z-10">
                                    <div className="flex flex-wrap items-center gap-4 mb-10">
                                        <div className="px-4 py-2 bg-white rounded-xl shadow-sm border border-slate-200">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Doğruluk Hedefi</span>
                                        ...
                                        <div className="px-4 py-2 bg-white rounded-xl shadow-sm border border-slate-100">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Hassasiyet Seviyesi</span>
                                            <span className="text-sm font-black text-[var(--color-primary-red)]">LOD {currentSectorData.lod}</span>
                                        </div>
                                    </div>

                                    <h3 className="text-4xl font-black text-slate-900 mb-6 leading-tight">{currentSectorData.title} BIM Çözümü</h3>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                        ...
                                        <div>
                                            <h4 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-6 flex items-center gap-2">
                                                <CheckCircle2 className="w-4 h-4 text-green-500" /> Uygulanan Hizmetler
                                            </h4>
                                        ...
                                        <div className="space-y-8">
                                            <div>
                                                <h4 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-4 flex items-center gap-2">
                                                    <FileCheck className="w-4 h-4 text-blue-500" /> Modeller & Çizimler
                                                </h4>
                                                <div className="flex flex-wrap gap-2">
                                                    {[...currentSectorData.deliverables.models, ...currentSectorData.deliverables.drawings].map((item, i) => (
                                                        <span key={i} className="px-3 py-1 bg-white border border-slate-200 rounded-lg text-[10px] font-black font-mono text-slate-500 uppercase">
                                                            {item}
                                                        </span>
                                                    ))}
                                                </div>
                                    ...
                                    <div className="mt-16 pt-10 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6">
                                        <div className="flex flex-col gap-2">
                                            <p className="text-sm font-bold text-slate-400">Bu sektör hakkında daha fazla teknik bilgi ve standartları inceleyin.</p>
                                            <Link href={`/sektorler/${currentSectorData.slug}`} className="text-sm font-black text-slate-900 underline hover:text-[var(--color-primary-red)] transition-colors">
                                                Detaylı İncele & SSS
                                            </Link>
                                        </div>
                                        <Link href="/iletisim" className="px-8 py-4 bg-[var(--color-primary-red)] text-white font-black rounded-2xl shadow-xl shadow-red-900/20 hover:bg-black transition-all flex items-center gap-3">
                                            Görüşme Talebi <ArrowRight className="w-5 h-5" />
                                        </Link>
                                    </div>
                                </div>
                            </motion.div>
                ...
                <div className="mt-32">
                    <div className="flex items-center justify-between mb-12">
                        <h4 className="text-2xl font-black text-slate-900 uppercase tracking-tight">İlgili Vaka Analizleri</h4>
                        <Link href="/referanslar" className="text-xs font-black text-[var(--color-primary-red)] uppercase tracking-widest hover:underline whitespace-nowrap">Tüm Projeleri Gör &rarr;</Link>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {relatedProjects.length > 0 ? (
                            relatedProjects.map(project => (
                                <Link key={project.slug} href={`/referanslar/${project.slug}`} className="group hover-lift active-shrink block h-full">
                            ...
                            <p className="text-slate-400 font-bold italic col-span-full py-10 bg-slate-50 rounded-[2rem] text-center border border-dashed border-slate-200">Bu sektör için henüz bir vaka analizi yayınlanmadı.</p>
                        )}
                    </div>
                </div>
            </div>
